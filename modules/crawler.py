...
async def robots(target):
	...
	print(G + '[+]' + C + ' Mencari untuk robots.txt' + W, end = '')

	try:
		r_rqst = requests.get(r_url, headers=user_agent, verify=False, timeout=10)
		r_sc = r_rqst.status_code
		if r_sc == 200:
			print(G + '['.rjust(9, '.') + ' Ketemu ]' + W)
			print(G + '[+]' + C + ' Mengekstrak Tautan Robot', end = '')
			r_page = r_rqst.text
			r_scrape = r_page.split('\n')
			for entry in r_scrape:
				if (entry.find('Menolak') == 0 or
					entry.find('Mengizinkan') == 0 or
					entry.find('Peta Jelajah') == 0):
			...
			print(R + '['.rjust(9, '.') + ' Tidak ditemukan ]' + W)
		else:
...
async def sitemap():
	...
	print(G + '[+]' + C + ' Mencari untuk sitemap.xml' + W, end = '')
	try:
		sm_rqst = requests.get(sm_url, headers=user_agent, verify=False, timeout=10)
		sm_sc = sm_rqst.status_code
		if sm_sc == 200:
			print(G + '['.rjust(8, '.') + ' Ditemukan ]' + W)
			print(G + '[+]' + C + ' Mengekstrak tautan sitemap', end = '')
			...
			print(R + '['.rjust(8, '.') + ' Tidak ditemukan ]' + W)
		else:
...
async def css(target):
	...
	print(G + '[+]' + C + ' Mengkstrak tautan CSS' + W, end = '')
...
async def js(target):
	...
	print(G + '[+]' + C + ' Mengekstrak tautan Javascript' + W, end = '')
...
async def internal_links(target):
	...
	print(G + '[+]' + C + ' Mengekstrak tautan Masuk' + W, end = '')
...
async def external_links(target):
	...
	print(G + '[+]' + C + ' Mengekstrak tautan Keluar' + W, end = '')
...
async def images(target):
	...
	print(G + '[+]' + C + ' Mengekstrak Gambar' + W, end = '')
...
async def sm_crawl():
	...
	print(G + '[+]' + C + ' Merobek Peta Jelajah' + W, end = '')
...
async def js_crawl():
	...
	print(G + '[+]' + C + ' Merobek Javascript' + W, end = '')
...
def out(target, output, data):
	...
	print('\n' + G + '[+]' + C + ' Jumlah tautan unik yang sudah diekstrak : ' + W + str(len(total)))

	if output != 'None':
		if len(total) != 0:
			data['module-Crawler'] = {'Jumlah tautan unik yang sudah diekstrak': str(len(total))}
			try:
