	...
	while True:
		try:
			try:
				...
				print(G + hop_index.ljust(7) + C + hop_addr.ljust(17) + W + hop_host)
				if output != 'None':
					collect.setdefault('Hasil', []).append([str(hop_index), str(hop_addr), str(hop_host)])
				continue
		finally:
			tx.close()
			break
...
def troute(ip, mode, port, tr_tout, output, data):
	...
	print('\n' + G + '[+]' + C + ' Jalur    : ' + W + str(port))
	print(G + '[+]' + C + ' Waktu Habis : ' + W + str(tr_tout))

	if mode == 'ICMP':
		print('\n' + Y + '[!]' + Y + ' Memulai ICMP Traceroute...' + W)
		icmp_trace(ip, tr_tout, output, collect)
	elif mode == 'UDP':
		print('\n' + Y + '[!]' + Y + ' Memulai UDP Traceroute...' + W)
		udp_trace(ip, port, tr_tout, output, collect)
	elif mode == 'TCP':
		print('\n' + Y + '[!]' + Y + ' Memulai TCP Traceroute...' + W)
		tcp_trace(ip, port, tr_tout, output, collect)
	else:
		print('\n' + R + '[-]' + C + ' Kesalahan Memilih Mode!' + W)
	
	if output != 'None':
		collect['Protocol'] = mode
		collect['Jalur'] = str(port)
		collect['Waktu Habis'] = str(tr_tout)
